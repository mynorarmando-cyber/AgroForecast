# ============================================================
# AGROFORECAST - CÓDIGO COMPLETO Y ACTUALIZADO
# ============================================================
import io
...
# ============================================================
# CURVAS Y PROTECCIÓN CONTRA PANTALLAS EN BLANCO
# ============================================================
def cycle_curve_data(d, vegetable):
    ...
    if len(cycle_keys) > 0:
        weekly = (
            x.groupby(cycle_keys + ["SemanaRelativa"], as_index=False)
            .agg(Kilos=("Kilos", "sum"))
        )
        totals = weekly.groupby(cycle_keys)["Kilos"].transform("sum")
    else:
        weekly = x.groupby(["SemanaRelativa"], as_index=False).agg(Kilos=("Kilos", "sum"))
        totals = weekly["Kilos"].sum()

    weekly["Pct"] = weekly["Kilos"] / totals.replace(0, np.nan)

    if len(cycle_keys) > 0 and "AñoCosecha" in x.columns:
        cycle_year = x.groupby(cycle_keys)["AñoCosecha"].first().reset_index()
        weekly = weekly.merge(cycle_year, on=cycle_keys, how="left")

    max_year = int(x["AñoCosecha"].max()) if "AñoCosecha" in x.columns and not x["AñoCosecha"].isna().all() else 2026
    weekly["Peso"] = weekly["AñoCosecha"].apply(lambda y: recency_weight(y, max_year)) if "AñoCosecha" in weekly.columns else 1.0
...
def recommended_curve(d, cycles, vegetable):
    try:
        ...
        try:
            ...
            if ds and "recommended" in ds:
                max_weeks = int(ds["recommended"])
        except Exception:
            pass
        ...
        for week, g in weekly.groupby("SemanaRelativa"):
            ...
            vals = g["Pct"].dropna().values
            weights = g["Peso"].dropna().values if "Peso" in g.columns else np.ones(len(vals))
            
            if len(vals) == 0:
                continue

            recent_mask = (g["AñoCosecha"] >= g["AñoCosecha"].max() - RECENT_YEARS).values if "AñoCosecha" in g.columns else np.zeros(len(vals), dtype=bool)

            rows.append({
                "Semana": int(week),
                "Historico": weighted_quantile(vals, .50, np.ones(len(vals))),
                "Reciente": weighted_quantile(vals[recent_mask], .50, np.ones(len(vals[recent_mask]))) if recent_mask.any() and len(vals[recent_mask]) > 0 else np.nan,
                "P25": weighted_quantile(vals, .25, weights[:len(vals)]),
                "P50": weighted_quantile(vals, .50, weights[:len(vals)]),
                "P75": weighted_quantile(vals, .75, weights[:len(vals)]),
                "N": int(len(g)),
            })
        ...
        st.error(f"Error generando curva recomendada: {e}")
        return pd.DataFrame()
...
def build_necesidades(d, cycles):
    ...
    for veg in sorted(cycles["Referencia"].dropna().unique()):
        ...
        curve = recommended_curve(d, cycles, veg)
...
def forecast(cycles, d, vegetable, area, first_harvest, scenario):
    ...
    ds = duration_analysis(cycles, vegetable)
    curve = recommended_curve(d, cycles, vegetable)
...
with tabs[1]:
    ...
    ys = yield_stats(cycles, veg)
    ds = duration_analysis(cycles, veg)
    curve = recommended_curve(data, cycles, veg)
...
# ============================================================
# PLAN & CALIDAD (Pestañas adicionales 5 y 6)
# ============================================================
with tabs[5]:
    st.subheader("🗓️ Plan de Siembra y Cosecha Masivo")
    st.info("Módulo auxiliar para carga o generación de planes por lote.")

with tabs[6]:
    st.subheader("🧪 Calidad y Validación de Datos")
    st.write(f"Registros en bruto analizados: {len(data):,}")
    st.write(f"Total de ciclos generados: {len(cycles):,}")
